...
Flujo mínimo para emitir y confirmar una factura:

1. **Login** — `POST /auth/login` → guardar el `access_token` y enviarlo como `Authorization: Bearer <token>` en todas las peticiones siguientes.
2. **Crear** — `POST /facturas` con `metodoPago`, `almacenId` e `items` (mínimo 1). Nace en estado `edicion`.
3. **Editar** (opcional, mientras esté en `edicion`) — `PATCH /facturas/:id` con solo los campos que cambian.
4. **Terminar** — `PATCH /facturas/:id/terminar` → pasa a `terminada` (cierra la edición normal, todavía sin tocar inventario).
5. **Confirmar** — `PATCH /facturas/:id/confirmar` → pasa a `confirmada`, descuenta stock y queda en firme.
...

{
  ...
  "items": [
    { "id": "i1", "productoId": "6ab335564ed6659d14298a71", "productoNombre": "Arroz",
      "cantidad": 2, "precio": 100, "costo": 60, "descuentoPct": 0, "descuentoMonto": 0, "recargo": 0 }
  ]
}
```
...
```http
PATCH /facturas/FAC-000005/confirmar
Authorization: Bearer <token>
```

Cada paso devuelve el objeto [`Factura`](#el-objeto-factura-lo-que-devuelve-el-servidor) completo con el nuevo `estado`. El resto de este documento cubre los detalles: matriz de edición, errores exactos, cancelación/anulación y casos borde.
...
Todas las rutas de `/facturas` requieren un JWT válido (`Authorization: Bearer <token>`), el mismo emitido por `POST /auth/login`. Sin token, o con uno inválido/expirado, el servidor responde `401`.
...
## Estados y transiciones de una factura
...
| `confirmada` | Factura en firme. Descuenta inventario (ver [Movimientos de inventario](#movimientos-de-inventario-al-confirmar--cancelar)).                                                | Solo `impreso` (marcarla como impresa no cambia nada fiscal).                                                                                                        |
...
## `PATCH /facturas/:id/terminar` — Cerrar la edición normal

- **Roles:** `administrador`, `gerente`, `facturador`.
- **Origen permitido:** `edicion`.
- **Efecto:** pasa a `terminada`. No toca inventario.
- **Éxito `200`:** la `Factura` con `estado: "terminada"`.
- **Errores:** `401`, `403`, `404` (no existe), `409` (no estaba en `edicion`).
...
```json
{
  "statusCode": 409,
  "error": "Conflict",
  "message": "La factura FAC-000005 esta en estado \"confirmada\" y no puede pasar a \"terminada\""
}
```

## `PATCH /facturas/:id/editar` — Volver a edición

- **Roles:** `administrador`, `gerente`, `facturador`.
- **Origen permitido:** `terminada`.
- **Efecto:** pasa a `edicion`. No toca inventario.
- **Éxito `200`:** la `Factura` con `estado: "edicion"`.
- **Errores:** `401`, `403`, `404`, `409` (no estaba en `terminada`).

## `PATCH /facturas/:id/confirmar` — Confirmar (descuenta inventario)

- **Roles:** `administrador`, `gerente`, `facturador`.
- **Origen permitido:** `terminada`.
- **Efecto:** pasa a `confirmada`. Descuenta stock y registra Kardex (ver [Movimientos de inventario](#movimientos-de-inventario-al-confirmar--cancelar)). Si falla, la factura vuelve a `terminada`.
- **Éxito `200`:** la `Factura` con `estado: "confirmada"`, `inventarioAplicado: true`.
- **Errores:** `401`, `403`, `404`, `409` (no estaba en `terminada`, **o** falló el descuento de stock — ver abajo).

## `PATCH /facturas/:id/cancelar` — Cancelar (reversa el inventario)

- **Roles:** `administrador`, `gerente`, `facturador`.
- **Origen permitido:** `confirmada`.
- **Efecto:** pasa a `cancelada`. Si la factura tiene `inventarioAplicado: true`, devuelve el stock y registra Kardex (ver abajo).
- **Éxito `200`:** la `Factura` con `estado: "cancelada"`, `inventarioRevertido: true` (si había stock que devolver).
- **Errores:** `401`, `403`, `404`, `409` (no estaba en `confirmada`, o hay una confirmación en curso sobre la misma factura — ver [Notas operativas](#notas-operativas-backend)).

## `PATCH /facturas/:id/anular` — Anular

Sin cuerpo. Solo desde `edicion` o `terminada`. La factura **no se borra**: queda con `estado: "anulada"` para conservar el registro, y su `numero`/`id` no se vuelve a usar nunca.

- **Roles:** `administrador`, `gerente`, `facturador`.
- **Origen permitido:** `edicion`, `terminada`.
- **Efecto:** pasa a `anulada`. No toca inventario.
...
- **Éxito `200`:** la `Factura` con `estado: "anulada"`.
- **Errores:** `401`, `403`, `404` (no existe), `409` (ya está `confirmada`, `cancelada` o `anulada`).

## `DELETE /facturas/:id` — Alias de anular

Hace **exactamente lo mismo** que `PATCH /facturas/:id/anular`: anula la factura sin borrarla. Mismos roles, respuestas y errores.

---

### Movimientos de inventario al confirmar / cancelar
...
Si algún producto no puede descontarse (stock insuficiente, no existe, está inactivo o pertenece a otro almacén que el de la factura), se revierten los descuentos ya hechos, la factura vuelve a `terminada` y la respuesta es `409`. El mensaje dice si el producto no existe, si está inactivo, si ya no pertenece al almacén de la factura (se movió), o el stock disponible frente al solicitado; si el stock ya alcanza al releerlo (otro proceso lo cambió), dice que el stock cambió durante la confirmación y pide reintentar:

```json
{
  "statusCode": 409,
  "error": "Conflict",
  "message": "No se puede confirmar la factura FAC-000012: stock insuficiente para el producto \"Tornillo\" (665f1c...): disponible 1, solicitado 3"
}
```

Dos `confirmar` simultáneos sobre la misma factura: solo uno la reclama; el otro recibe el `409` de transición inválida, así que el stock nunca se descuenta dos veces. Un `cancelar` que llega mientras una confirmación todavía está en curso recibe:

```json
{
  "statusCode": 409,
  "error": "Conflict",
  "message": "La factura FAC-000012 se está confirmando; reintente"
}
```

> Para el frontend, el `409` de "se está confirmando; reintente" se maneja igual que cualquier otro `409` de transición: mostrar el mensaje y dejar reintentar.
...
| `facturadoPor`                                        | `object`  | El usuario (`Facturador`) autenticado que emitió la factura. Lo toma el servidor del token JWT, nunca del cuerpo de la petición (ver [Facturado por](#facturado-por)).                                                             |
...
| `estado`                                              | `string`  | `"edicion"`, `"terminada"`, `"confirmada"`, `"cancelada"` o `"anulada"`. Toda factura nace `"edicion"` (ver [Estados y transiciones](#estados-y-transiciones-de-una-factura)).                                                     |
| `revision`                                            | `number`  | **Server-controlado, nunca en el `POST`/`PATCH`:** contador de concurrencia optimista. Se incrementa en cada `PATCH /facturas/:id` y en cada transición de estado. Ver [Concurrencia al editar](#concurrencia-al-editar-revision). |
| `inventarioAplicado`                                  | `boolean` | **Server-controlado:** `true` solo si al confirmar esta factura se descontó el stock.                                                                                                                                              |
| `inventarioRevertido`                                 | `boolean` | **Server-controlado:** `true` cuando al cancelar se devolvió ese stock.                                                                                                                                                            |
| `inventarioEnProceso`                                 | `boolean` | **Server-controlado:** `true` solo mientras `confirmar`/`cancelar` mueven stock. Si queda puesto sin una petición en curso, ver [Notas operativas](#notas-operativas-backend).                                                     |
...
Los tres son opcionales tanto al crear (`POST`) como al editar (`PATCH`, ver [Editar datos no fiscales](#patch-facturasid--editar-una-factura)).
...
> **El almacén debe tener `codigo` configurado.** Si el almacén elegido todavía no tiene `codigo`, configurarlo con `PATCH /almacen/:id` (`{ "codigo": "ALM-001" }`) antes de facturar desde él, o el servidor responde `422`.
...
> Si el almacén todavía no tiene `codigo`, configurarlo primero con `PATCH /almacen/:id` para poder facturar desde él.
...
```json
{
  "statusCode": 409,
  "error": "Conflict",
  "message": "La factura FAC-000005 esta en estado \"terminada\": no se puede modificar concepto"
}
```
...
### Concurrencia al editar (`revision`)
...
```json
{
  "statusCode": 409,
  "error": "Conflict",
  "message": "La factura FAC-000005 cambio mientras se editaba; reintente"
}
```

Esto puede pasar aunque el `estado` no haya cambiado (por ejemplo, dos usuarios editando `concepto` en `edicion` al mismo tiempo): sin esta protección, la segunda escritura pisaría silenciosamente los totales/almacén/cliente recalculados por la primera (lost update). Ante este `409`, el cliente debe volver a pedir `GET /facturas/:id` (para tener el `estado`/`revision`/datos actuales) y reintentar el `PATCH` con esa información fresca — nunca reintentar a ciegas con el cuerpo original.
...
```json
{
  ...
  "error": "Bad Request",
  "message": [
    "property numero should not exist",
...
```json
{
  "statusCode": 404,
  "error": "Not Found",
  "message": "Factura con ID FAC-999999 no encontrada"
}
```
...
```json
{
  "statusCode": 409,
  "error": "Conflict",
  "message": "La factura FAC-000006 esta en estado \"terminada\": no se puede modificar concepto, items"
}
```
...
```json
{
  "statusCode": 409,
  "error": "Conflict",
  "message": "La factura FAC-000006 esta en estado \"cancelada\" y no puede pasar a \"anulada\""
}
```
...
```json
{
  ...
  "message": "El almacén \"Almacén Central\" no tiene código configurado"
}
```
...
- [ ] Enviar siempre `almacenId` (el `_id` del almacén) y el `productoId` real (`_id` de Mongo) de cada item. El almacén debe tener `codigo` configurado (`PATCH /almacen/:id`) o el `POST` responde `422`.
- [ ] Recordar que la factura nace en `edicion`: hace falta `terminar` y luego `confirmar` para que descuente inventario.
...
- [ ] Manejar el `409` al editar (el estado actual no permite alguno de los campos enviados, o la factura cambió mientras se editaba — ver [Concurrencia al editar](#concurrencia-al-editar-revision)) o al pedir una transición de estado (`terminar`/`editar`/`confirmar`/`cancelar`/`anular`) que no aplica desde el estado actual.
- [ ] Al confirmar, manejar el `409` de stock insuficiente / producto inactivo / de otro almacén (la factura sigue en `terminada`, ver [Movimientos de inventario](#movimientos-de-inventario-al-confirmar--cancelar)).
- [ ] Ante el `409` de concurrencia (incluido el `409` "se está confirmando; reintente" al cancelar), volver a pedir `GET /facturas/:id` y reintentar, en vez de reintentar a ciegas con el cuerpo original.
...
Detalle interno para quien opera el servidor; el frontend no necesita nada de esta sección más allá de reintentar ante un `409`.

**Ids guardados como texto:** `Producto.almacen` puede estar guardado como ObjectId o como el texto hex que envió el cliente (por ejemplo, productos creados con `POST /producto`). Las comprobaciones de almacén y de estado inactivo (al crear/editar la factura y al confirmar) aceptan el ObjectId y su hex en minúsculas o mayúsculas, así que un producto del mismo almacén nunca recibe un `409` falso. Un hex con mayúsculas y minúsculas mezcladas no se reconoce al confirmar.

**Marcador `inventarioEnProceso`:** mientras `confirmar` o `cancelar` mueven stock, la factura lleva `inventarioEnProceso: true`; se quita en la misma escritura condicional que termina la operación (éxito, o vuelta a `terminada` si la confirmación falla). Un `cancelar` que llega mientras una confirmación todavía está descontando stock **no** puede reclamar la factura y recibe el `409` de "se está confirmando; reintente", sin tocar el stock. Así nunca se devuelve stock que todavía no se descontó.
